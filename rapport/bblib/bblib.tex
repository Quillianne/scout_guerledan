...
\subsubsection{Architecture matérielle et réseau}

Le projet repose sur une flotte de trois bateaux autonomes \textit{BlueBoat} de Blue Robotics (figure~\ref{fig:blueboat}), et d'une station de base terrestre. Chaque BlueBoat embarque un \textbf{Raspberry Pi} exécutant \textbf{BlueOS}, le système d'exploitation embarqué développé par Blue Robotics, ainsi qu'un \textbf{Pixhawk} (contrôleur de vol) qui exécute le firmware \textbf{ArduRover}. Le Pixhawk et le Raspberry Pi communiquent en local via une liaison USB/UART. La station de base et les trois bateaux sont reliés par un réseau \textbf{WiFi local} sur le sous-réseau 192.168.2.0/24 (figure~\ref{fig:blueboat_stack}).

\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{../images/blueboat.png}
  \caption{Un BlueBoat de Blue Robotics utilisé dans le projet.}
  \label{fig:blueboat}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=0.7\textwidth]{../images/blueboat_stack.png}
  \caption{Pile logicielle embarquée sur un BlueBoat : le Pixhawk exécute ArduRover et communique avec le Raspberry Pi (BlueOS) via MAVLink série.}
  \label{fig:blueboat_stack}
\end{figure}

\subsubsection{Le protocole MAVLink}

\textbf{MAVLink} (\textit{Micro Air Vehicle Link}) est un protocole de communication léger, binaire et orienté messages, conçu pour les systèmes aériens et terrestres autonomes. Il est le standard de facto de l'écosystème ArduPilot/PX4.

MAVLink définit un ensemble de \textbf{messages standardisés} — par exemple \texttt{ATTITUDE} pour l'orientation (roll, pitch, yaw), \texttt{GLOBAL\_POSITION\_INT} pour la position GPS, \texttt{RC\_CHANNELS\_OVERRIDE} pour la commande des moteurs, ou encore \texttt{COMMAND\_LONG} pour l'envoi d'ordres tels que l'armement ou le changement de mode. Chaque message possède un identifiant numérique, une structure binaire fixe et est adressé par un couple (\texttt{system\_id}, \texttt{component\_id}) qui identifie le véhicule et le composant cible.

Dans l'usage traditionnel, MAVLink transite sur un \textbf{socket UDP ou TCP} : le logiciel de contrôle (Mission Planner, QGroundControl) s'abonne au flux de messages sur le port 14550 et doit maintenir une connexion active.

\subsubsection{mavlink2rest : MAVLink accessible via HTTP}

\textbf{mavlink2rest} est un serveur léger intégré à BlueOS qui fait le pont entre le flux MAVLink binaire du Pixhawk et une \textbf{API HTTP/REST}. Son fonctionnement est le suivant :
...
\subsubsection{Pourquoi mavlink2rest plutôt que pymavlink ?}

La bibliothèque Python officielle \texttt{pymavlink} permet d'ouvrir des connexions MAVLink bas niveau (UDP, TCP, série) et d'envoyer/recevoir des messages binaires. C'est l'approche classique. Cependant, dans notre contexte, elle présente plusieurs inconvénients majeurs.
...
\subsubsection{Besoin d'une bibliothèque commune}
...
\subsubsection{\texttt{MavlinkLink} — couche de communication REST}
...
\subsubsection{\texttt{IMU} — lecture d'attitude}
...
\texttt{get\_attitude\_dict()} retourne un dictionnaire enrichi incluant \texttt{yaw\_deg}, le cap normalisé dans $[0°, 360°]$, directement exploitable par la couche Navigation.

\subsubsection{\texttt{GPS} — lecture de position}
...
\subsubsection{\texttt{MotorDriver} — commande des moteurs}
...
\subsubsection{\texttt{Navigation} — navigation autonome}
...
\[
  e = \psi_{\text{cible}} - \psi_{\text{actuel}} \quad (\text{normalisée dans } {[-180°, 180°]})
\]
\[
...
\subsubsection{\texttt{BlueBoatConfig} et \texttt{init\_blueboat()}}
...
La classe \texttt{BlueBoatConfig} charge ce fichier avec des valeurs par défaut en cas d'absence. \texttt{init\_from\_config(boat\_id)} instancie en une seule ligne l'ensemble des composants d'un bateau (\texttt{MavlinkLink}, \texttt{IMU}, \texttt{GPS}, \texttt{MotorDriver}, \texttt{Navigation}). \texttt{init\_all()} parcourt tous les bateaux configurés et retourne un dictionnaire indexé par \texttt{boat\_id}, utilisé par les scripts de mission multi-bateaux.
...
\subsubsection{\texttt{HeartbeatManager} — maintien de la connexion GCS}
...
\subsubsection{\texttt{BoatMonitor} — acquisition des données}
...
\subsubsection{Interface Tkinter}
...
Pour permettre le développement et les tests sans accès physique aux bateaux, un serveur HTTP simulant l'API mavlink2rest a été développé dans \texttt{fake\_mavlink2rest.py}. Il démarrage un serveur HTTP léger par port (un par bateau simulé), chacun générant des données synthétiques :
\begin{itemize}
...
Ce simulateur a été utilisé lors de la semaine de formation pour tester rapidement les boucles de commande, valider \texttt{bblib} et \texttt{boat\_control\_gui.py}, et vérifier les scripts de mission multi-bateaux, sans mobiliser le matériel.

% ---------------------------------------------------------------------------
\subsection{Tests de la première semaine de formation}
\label{sec:bblib:tests}
% ---------------------------------------------------------------------------

\subsubsection{\texttt{test\_bblib.py} — tests CLI de la bibliothèque}
...
\subsubsection{Autres scripts de test}

Plusieurs scripts complémentaires ont été développés durant la semaine de formation :
...
\begin{figure}[h]
  ...
  \caption{Trace GPS d'un test de formation en triangle avec les trois bateaux.}
  \label{fig:test_formation}
\end{figure}

\subsubsection{Résultats et observations}
